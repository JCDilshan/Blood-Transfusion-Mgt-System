...
class MyPdf extends FPDF
{
	...
	function Header()
	{
		...
		$this->Cell(200, 10, 'REGISTERED NURSE', 0, 0, 'C');
		$this->Ln();
	...
	function TableHeading()
	{
		...
		$this->Cell(25, 10, 'Nurse ID', 1, 0, 'C');
		$this->Cell(75, 10, 'Nurse Name', 1, 0, 'C');
	...
	function TableBody()
	{
		...
		$result = $nurse_object->get_AllNurse();
		...
		if ($result->num_rows > 0) {
			...
			while ($row = $result->fetch_assoc()) {
				$this->Cell(25, 10, $row['nurse_id'], 1, 0, 'C');
				$this->Cell(75, 10, $row['fname'] . " " . $row['lname'], 1, 0, 'C');
...
$date = date("y-m-d");
$d1 = $date . "";
$filename = "ListedNurseReport_$d1.pdf";
