...
class MyPdf extends FPDF
{
	...
	function Header()
	{
		...
		$this->Cell(200, 10, 'REGISTERED DOCTORS', 0, 0, 'C');
		$this->Ln();
	...
	function TableHeading()
	{
		...
		$this->Cell(25, 10, 'Doctor ID', 1, 0, 'C');
		$this->Cell(75, 10, 'Doctor Name', 1, 0, 'C');
	...
	function TableBody()
	{
		...
		$doc_object = new Doctor($conn);
		$result = $doc_object->get_Alldoctors();
		...
		if ($result->num_rows > 0) {
			...
			while ($row = $result->fetch_assoc()) {
				$this->Cell(25, 10, $row['doc_id'], 1, 0, 'C');
				$this->Cell(75, 10, "Dr. " . $row['fname'] . " " . $row['lname'], 1, 0, 'C');
...
$date = date("y-m-d");
$d1 = $date . "";
$filename = "ListedDocReport_$d1.pdf";
